...
export function PostgresCustomerOnboardingForm({ pending, submit, error }: { pending: boolean; submit: (input: { legalName: string; registrationIdentifier: string }) => void; error?: string | null }) {
  ...
  const onSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    submitOnce({ legalName: String(data.get("legalName")).trim(), registrationIdentifier: String(data.get("registrationIdentifier")).trim() });
  };

  return <form onSubmit={onSubmit} className="grid gap-4 p-5">
    <p className="text-xs leading-5 text-black/60">Create the authoritative customer record from verified onboarding evidence only. This creates no payment instruction and no automatic KYC/KYB disposition.</p>
