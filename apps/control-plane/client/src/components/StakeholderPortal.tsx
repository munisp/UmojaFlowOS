...
function ExternalEvidencePortal({ role }: { role: "provider_contact" | "cbn_liaison" }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[1fr_0.9fr]">
    <section className="uf-panel"><div className="border-b border-black/20 px-5 py-4"><p className="uf-kicker">Assigned evidence scope</p><h2 className="mt-1 text-lg font-black uppercase tracking-[-0.04em]">Recorded assignments</h2></div>
      {assignments.isLoading ? <p className="px-5 py-7 text-sm text-black/55">Loading assigned records…</p> : rows.length ? <div className="divide-y divide-black/10">{rows.map((row: any) => <div className="px-5 py-4" key={row.id}><p className="text-sm font-black uppercase">{role === "provider_contact" ? row.counterpartyLegalName : row.legalEntityName}</p><p className="mt-1 text-sm text-black/60">{role === "provider_contact" ? `Onboarding stage: ${row.onboardingStage ?? "not recorded"}` : `${row.productName} · ${row.track} · dossier ${row.dossierStatus}`}</p><p className="mt-2 text-[10px] font-black uppercase tracking-[0.12em] text-[#e11919]">Assignment status: {row.status}</p></div>)}</div> : <p className="px-5 py-7 text-sm leading-6 text-black/60">No active assignment is recorded. An administrator must assign an approved counterparty or CBN dossier before this portal accepts evidence.</p>}</section>
...
export function StakeholderPortalGallery() {
  const roles: OperatorRole[] = ["admin", "compliance_officer", "treasury_operator", "auditor", "provider_contact", "cbn_liaison"];
  return <section className="uf-panel" data-testid="stakeholder-portal-gallery"><div className="border-b border-black/20 px-5 py-4"><p className="uf-kicker">Stakeholder portal gallery</p><h2 className="mt-1 text-xl font-black uppercase tracking-[-0.05em]">Six role-specific workspaces</h2><p className="mt-2 max-w-4xl text-sm leading-6 text-black/60">This is a visual map of the available portal experiences, not role switching. The authenticated role assignment remains the only source of actual controls and evidence access.</p></div><div className="grid divide-y divide-black/10 sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-3">{roles.map((role, index) => { const portal = portalCards[role]; return <article key={role} className="min-h-52 p-5"><div className="flex items-start justify-between gap-4"><span className="flex h-9 w-9 items-center justify-center bg-[#e11919] text-xs font-black text-white">0{index + 1}</span><span className="border border-black/20 px-2 py-1 text-[9px] font-black uppercase tracking-[0.13em] text-black/55">{portal.label}</span></div><h3 className="mt-6 text-base font-black uppercase tracking-[-0.04em]">{portal.title}</h3><p className="mt-3 text-sm leading-6 text-black/60">{portal.boundary}</p><div className="mt-5 border-l-2 border-[#e11919] pl-3 text-[10px] font-black uppercase tracking-[0.12em] text-black/60">{role === "provider_contact" ? "Assigned evidence only" : role === "cbn_liaison" ? "Not submitted / non-licensing" : role === "auditor" ? "Read-only assurance" : "Role-scoped controls"}</div></article>; })}</div></section>;
}
