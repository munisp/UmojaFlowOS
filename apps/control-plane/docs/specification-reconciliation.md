...
| RQ-16 | No provider represented as active without approved credentials | Implemented | Exactly one activation path exists and it is verified rather than declared: an integration reaches `active` only when a real request to its configured endpoint returns 2xx, using a named deployment secret. A rejected credential, server error, redirect, unreachable host, or timeout each record an explicit failure and leave it inactive. The credential value itself is refused by the interface, the API, and the schema. The service bridge remains disabled by default with no localhost fallback. `providerActivationGate.test.ts` asserts the single path and its precondition, verified by a negative control. |
