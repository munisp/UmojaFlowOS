...
## Second reset: language toolchains and Python packages

A later reset removed the Go and Rust toolchains and every locally installed
Python package while PostgreSQL and Ollama survived. `cargo`, `go`, `pytest`,
`fastapi`, `paddleocr`, and `docling` were all absent, so `make check` could not
run at all and the Go, Rust, and Python suites were unverifiable:
...
sudo apt-get install -y build-essential   # rustc requires a system linker
sudo pip3 install pytest fastapi httpx uvicorn paddleocr docling
```

Restoring the toolchains exposed two genuine defects that had been invisible
while they were missing, which is the main lesson from this reset: an
unrunnable suite is not a passing suite.

| Defect | Detail |
| --- | --- |
| Two Ollama digest-drift regressions were failing for the wrong reason | Their stub HTTP client exposed only `post`, but the adapter had since gained a `GET /api/tags` digest fallback, so both tests failed with `AttributeError: 'Client' object has no attribute 'get'` instead of exercising the drift they exist to detect. The stubs now answer the tag inventory as well. |
| The Rust monitoring and counterparty-risk modules were never declared in `lib.rs` | They compiled nowhere, so 21 of their tests never ran and the two HTTP routes the control-plane bridge calls did not exist. Registering the modules raised the Rust suite from 13 to 34 tests. |

After re-provisioning, `make check` passes across Go, Rust, Python, and
TypeScript, and the toolchain versions in use are Go 1.23.6, cargo 1.97.1, and
Python 3.12 with PaddleOCR 3.7.0 and Docling 2.120.3.

## Standing risk

Local PostgreSQL, the Ollama runtime, the language toolchains, locally installed
Python packages, and any uncommitted monorepo state do not survive a sandbox
reset. The mitigations are that every schema change lives in a numbered
migration, the privilege model lives in `grants.sql`, model digests are recorded
in the inventory document, this note records the exact toolchain provisioning
commands, and the managed project is checkpointed after each unit of work.
