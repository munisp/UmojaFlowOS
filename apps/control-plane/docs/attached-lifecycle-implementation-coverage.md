...
The current active-code mappings are deliberately narrow. The Go payment engine can emit a redacted Temporal workflow outcome only after its canonical decision recorder succeeds, and a redacted Permify decision only after the authorization result has already been determined; neither publisher can alter workflow or authorization authority. The Python Dapr subscriber maps those events only to `temporal_workflow` and `permify_authorization`, respectively, then hashes the internal correlation before catalog projection. Stablecoin exposure projects one record per reconciled corridor/asset only when `UMOJA_LAKEHOUSE_PROJECT_STABLECOIN_EXPOSURE=true`; the projection contains no amount, account reference, or source reference and does not change the public exposure response. All storage writes remain disabled until the separately configured immutable lakehouse writer is available.

Canonical PostgreSQL controls now join the same governed path through migration 0014. Creation, gate-decision, and recertification triggers write only a SHA-256 correlation, the stage/gate/cycle outcome, observation time, and `authoritative: false` into `control_evidence_outbox` in the same transaction as the real onboarding decision. An authenticated cron-only drain is disabled unless both the catalog endpoint and a dedicated PostgreSQL-control token are configured. It locks pending rows, sends only the source-bound catalog form, records one bounded failure attempt without changing the underlying control decision, and marks a successful response delivered so it cannot be sent again. This is an outbox for analytics evidence, not an operational workflow, settlement, or approval mechanism.
