...
async function acquireLeader(environment: NodeJS.ProcessEnv = process.env): Promise<(() => Promise<void>) | null> {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    try {
      ...
      return async () => {
        try {
          await lockedClient.query("SELECT pg_advisory_unlock($1)", [ADVISORY_LOCK_KEY]);
          lockedClient.release();
        } catch (error) {
          // Never recycle a session whose advisory-lock state is uncertain.
          lockedClient.release(error as Error);
          throw error;
        }
      };
    } catch (error) {
      lastError = error;
      if (client) client.release(error as Error);
      if (!retryableDatabaseError(error) || attempt === attempts) throw error;
      await delay(Math.min(backoff * 2 ** (attempt - 1), MAX_LOCK_RETRY_BACKOFF_MS));
    }
  }
  throw lastError instanceof Error ? lastError : new Error("advisory lock acquisition failed");
}
...
export async function runSegregationOfDutiesMonitorRound(environment: NodeJS.ProcessEnv = process.env): Promise<SoDMonitorResult> {
  ...
  try {
    release = await acquireLeader(environment);
...
export const segregationOfDutiesMonitorConfiguration = {
  strictEnabled,
  configuredInterval,
  configuredSubject,
  configuredLockRetryAttempts,
  configuredLockRetryBackoff,
};
