...
const currentlyMappedBusinessTables = new Set(["counterparties", "counterpartyAuthorizations", "customers"]);
...
try {
  ...
  const mappedUsers = mapRoles(sourceUsers), counterparties = mapCounterparties(sourceCounterparties), counterpartyAuthorizations = mapCounterpartyAuthorizations(sourceCounterpartyAuthorizations), customers = mapCustomers(sourceCustomers);
  const sourceCounts = {};
  ...
  const sourceSnapshotSha256 = checksum({ userRoles: mappedUsers, businessTableCounts: sourceCounts, counterparties, counterpartyAuthorizations, customers });
  ...
  const businessReconciliations = [await migrateCounterparties(counterparties, runId), await migrateCounterpartyAuthorizations(counterpartyAuthorizations, runId), await migrateCustomers(customers, runId)];
