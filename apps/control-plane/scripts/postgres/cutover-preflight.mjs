...
try {
  ...
  const marketObservations = sourceMarketObservations.map(row => ({ id: deterministicUuid("marketObservations", row.id), integrationConnectionId: deterministicUuid("integrationConnections", row.integrationConnectionId), baseAsset: row.baseAsset, quoteAsset: row.quoteAsset, rate: String(row.rate), observedAt: asIso(row.observedAt), receivedAt: asIso(row.createdAt), sourceReference: row.sourceReference })).sort((a, b) => a.id.localeCompare(b.id));
  ...
  const paymentOrders = sourcePaymentOrders.map(row => { if (row.policyDecisionReference) throw new Error(`Cutover blocked: payment order ${row.id} has a legacy policyDecisionReference with no approved canonical policy-decision mapping`); return { id: deterministicUuid("paymentOrders", row.id), idempotencyKey: row.idempotencyKey, customerId: deterministicUuid("customers", row.customerId), beneficiaryId: deterministicUuid("beneficiaries", row.beneficiaryId), corridor: row.corridor, sourceCurrency: row.sourceCurrency, sourceAmount: String(row.sourceAmount), targetCurrency: row.targetCurrency, targetAmount: row.targetAmount === null ? null : String(row.targetAmount), status: row.status, providerFinalityReference: row.providerFinalityReference ?? null, createdBy: row.createdBy, createdAt: asIso(row.createdAt), updatedAt: asIso(row.updatedAt) }; }).sort((a, b) => a.id.localeCompare(b.id));
